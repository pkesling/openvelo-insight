...
class TestForecastService(unittest.TestCase):
    def test_is_day_normalized(self):
        tz = ZoneInfo("America/Chicago")
        start_local = dt.datetime(2025, 1, 1, 12, 0, tzinfo=tz)
        ...
        def fake_weather_hours(*_args, **_kwargs):
            return []

        def fake_air_hours(*_args, **_kwargs):
            return []
        ...
        conditions = get_bike_conditions_for_window(
            latitude=43.0,
            longitude=-89.0,
        ...
        self.assertIs(conditions.current.is_day, True)
